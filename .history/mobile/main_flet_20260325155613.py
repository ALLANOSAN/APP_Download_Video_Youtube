...
def main(page: ft.Page):
            ...
            if success and results:
                for item in results:
                    ...
                    card = ft.Container(
                        ...
                        on_click=lambda _, url=video_url, t=item.get('title'), img=item.get('thumbnail'), up=item.get('uploader'):
                            asyncio.create_task(handle_download(t, url, img, up)),
    ...
    async def update_library():
        if not sync_client.token:
            ...
            if not history:
                library_list.controls.append(ft.Text("Nenhum download sincronizado.", color=TEXT_DIM))
            for item in history:
    ...
    pass_input = ft.TextField(label="Senha", password=True, can_reveal_password=True, border_color=ACCENT)
    ...
    header = ft.Container(
        content=ft.Column([
            ft.Row([
                ...
                ft.IconButton(ft.icons.PERSON_ROUNDED, icon_color=TEXT_DIM, on_click=lambda _: show_login_dialog())
    ...
    def on_nav_change(e):
        ...
        if idx == 0:
            ...
            main_content.content = ft.Column([
                ft.Container(ft.Text("Biblioteca", size=28, weight=ft.FontWeight.BOLD), padding=20),
                library_list
            ], expand=True)
