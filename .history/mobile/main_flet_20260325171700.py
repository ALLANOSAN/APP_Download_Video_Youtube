...
def main(page: ft.Page):
    ...
    USE_BACKEND_DOWNLOAD = True  # fallback ao backend para yt-dlp/ffmpeg
    MOBILE_DOWNLOAD_DIR = Path.home() / "Downloads" / "YouTubeDownloader"
    ...
    # Seletores de download (modo + formatos)
    mode_select = ft.Dropdown(
        width=130,
        value="audio",
        options=[
            ft.dropdown.Option("audio", text="Áudio"),
            ft.dropdown.Option("video", text="Vídeo"),
        ],
        label="Modo",
    )

    audio_format_select = ft.Dropdown(
        width=130,
        value="mp3",
        options=[
    ...
    audio_quality_select = ft.Dropdown(
        width=130,
        value="192k",
        options=[
    ...
    video_quality_select = ft.Dropdown(
        width=130,
        value="720p",
        options=[
                    ...
                    card = ft.Container(
                        ...
                        up=item.get("uploader"): asyncio.create_task(
                            handle_download(
                                ...
                                up,
                                mode_select.value,
                                audio_format_select.value,
                                audio_quality_select.value,
                                video_quality_select.value,
                            )
                        ),
    ...
    async def handle_download(
        ...
        mode = mode or mode_select.value or "audio"
        audio_format = audio_format or audio_format_select.value or "mp3"
        audio_quality = audio_quality or audio_quality_select.value or "192k"
        video_quality = video_quality or video_quality_select.value or "720p"

        if mode == "video":
            show_message(f"Baixando vídeo: {title} ({video_quality})")
            default_filename = f"{title.replace(' ', '_')}.{video_quality}.mp4"
        else:
            show_message(f"Baixando áudio: {title} ({audio_format}, {audio_quality})")
            default_filename = f"{title.replace(' ', '_')}.{audio_format}"

        # Remover caracteres incompatíveis do nome do arquivo
        safe_name = "".join(c for c in default_filename if c.isalnum() or c in "._-")
        if not safe_name:
            safe_name = "downloaded_media"

        try:
            if USE_BACKEND_DOWNLOAD:
                payload = {
                    "url": url,
                    "mode": mode,
                ...
                async with httpx.AsyncClient(timeout=600) as client:
                    response = await client.post(f"{API_URL}/download", json=payload)

                if response.status_code != 200:
                    raise Exception(f"Falha no servidor ({response.status_code}): {response.text}")

                task_id = response.json().get("task_id")
                if not task_id:
                    raise Exception("Task ID não retornado")

                # Polling de progresso
                while True:
                    await asyncio.sleep(1)
                    progress_resp = await client.get(f"{API_URL}/download_progress/{task_id}")
                    if progress_resp.status_code != 200:
                        raise Exception("Erro ao consultar progresso")

                    progress_data = progress_resp.json()
                    status = progress_data.get("status")
                    pct = progress_data.get("percent", 0)
                    msg = progress_data.get("message", "")
                    show_message(f"{status}: {pct:.1f}% - {msg}")

                    if status == "done":
                        file_resp = await client.get(f"{API_URL}/download_file/{task_id}")
                        if file_resp.status_code != 200:
                            raise Exception(f"Erro ao baixar arquivo final: {file_resp.status_code}")

                        output_file = MOBILE_DOWNLOAD_DIR / safe_name
                        with open(output_file, "wb") as f:
                            f.write(file_resp.content)

                        show_message(f"Download concluído: {output_file}")
                        break

                    if status == "error":
                        raise Exception(f"Erro no servidor: {msg}")

                # Força refresh de biblioteca
            else:
                # Modo local (usando o VideoDownloader do dispositivo)
                if mode == "video":
                    success, message, _ = downloader.download_video(url, video_quality=video_quality)
                else:
                    success, message, _ = downloader.download_audio(url, audio_format=audio_format, audio_quality=audio_quality)

                if not success:
                    raise Exception(message)

                # salva em local com nome dinâmico
                output_file = MOBILE_DOWNLOAD_DIR / safe_name
                show_message(f"Download local concluído: {output_file}")
            ...
            show_message(f"Erro no download: {e}")
    ...
    header = ft.Container(
        content=ft.Column(
            [
                ...
                ft.Row(
                    [mode_select, audio_format_select, audio_quality_select, video_quality_select],
                    spacing=10,
                ),
                ft.Container(height=10),
                search_field,
