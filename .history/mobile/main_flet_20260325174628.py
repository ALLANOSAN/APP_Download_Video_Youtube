    ...
    async def handle_download(
        ...
        try:
            if USE_BACKEND_DOWNLOAD:
                ...
                while True:
                    ...
                    if status == "done":
                        ...
                        if file_resp.status_code != 200:
                            raise Exception(
                                f"Erro ao baixar arquivo final: {file_resp.status_code}"
                            )
                ...
                if mode == "video":
                    success, message, _ = downloader.download_video(
                        url, video_quality=video_quality
                    )
                else:
                    success, message, _ = downloader.download_audio(
                        url, audio_format=audio_format, audio_quality=audio_quality
                    )
