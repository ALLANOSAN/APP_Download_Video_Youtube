...
# Garante que o pacote src está disponível para imports absolutos e para análise estática (ty/pyright).
...
try:
    from src.downloader.video_downloader import (  # type: ignore[import-not-found]
        VideoDownloader,
        AUDIO_FORMATS,
        AUDIO_QUALITIES,
        VIDEO_QUALITIES,
    )
    from src.utils.settings import get_settings  # type: ignore[import-not-found]
    from src.utils.logger import get_logger  # type: ignore[import-not-found]
    from src.gui.styles import MAIN_STYLE, MODAL_STYLE, COLORS, SIZES  # type: ignore[import-not-found]
    from src.gui.components import SkeletonCard  # type: ignore[import-not-found]
except ImportError:
    from downloader.video_downloader import (  # type: ignore[import-not-found] # ty: ignore[unresolved-import]
        VideoDownloader,
        AUDIO_FORMATS,
        AUDIO_QUALITIES,
        VIDEO_QUALITIES,
    )
    from utils.settings import get_settings  # type: ignore[import-not-found] # ty: ignore[unresolved-import]
    from utils.logger import get_logger  # type: ignore[import-not-found] # ty: ignore[unresolved-import]
    from gui.styles import MAIN_STYLE, MODAL_STYLE, COLORS, SIZES  # type: ignore[import-not-found] # ty: ignore[unresolved-import]
    from gui.components import SkeletonCard  # type: ignore[import-not-found] # ty: ignore[unresolved-import]
...
class ThumbnailLabel(QLabel):
    ...
    def __init__(self, parent=None):
        ...
        self._skeleton.setGeometry(0, 0, 160, 90)
        self._skeleton.hide()
...
class SearchDialog(QWidget):
    ...
    def _setup_ui(self):
        ...
        # Skeleton Loader
        self.skeleton = SkeletonCard(self)
        self.skeleton.hide()
        layout.insertWidget(2, self.skeleton)
...
class YouTubeDownloaderApp(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        # Título
        title = QLabel("YouTube Music Pro")
        title.setFont(QFont("Segoe UI", 26, QFont.Weight.Bold))
        title.setAlignment(Qt.AlignmentFlag.AlignCenter)
        title.setStyleSheet(
            f"color: {COLORS['accent_primary']}; margin-top: 10px; margin-bottom: 5px;"
        )
        layout.addWidget(title)

        subtitle = QLabel("Advanced Music Downloader")
    ...
    def _on_detailed_info(
        ...
        if success and data:
            ...
            if views >= 1_000_000:
                ...
                views_str = f"{views / 1_000:.1f}k visualizações"
            else:
            ...
            uploader = data.get("uploader", "Artista Desconhecido")
            ...
            info_text = (
                ...
                f"<b style='font-size: 16px; color: {COLORS['accent_primary']};'>{data.get('title', 'Vídeo')}</b><br>"
                f"<span style='color: {COLORS['text_primary']}; font-weight: bold;'>{uploader}</span><br>"
                f"<span style='color: {COLORS['text_secondary']};'>{mins}:{secs:02d} • {views_str} • {upload_date}</span>"
                f"</div>"
            )
