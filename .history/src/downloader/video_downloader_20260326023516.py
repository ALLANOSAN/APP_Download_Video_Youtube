...
def resolve_binary(name: str, env_var: str) -> Path:
    ...
    # 2. Usa binários já empacotados no projeto (root/binaries)
    ...
    def download_video(
        ...
        for quality_option in candidates:
            ...
            try:
                ...
                if return_code == 0:
                    self.logger.info(
                        f"Download de vídeo concluído: {url} (qualidade {quality_option})"
                    )
                    return True, "Download concluído", str(self.output_dir)

                self.logger.warning(
                    f"Falha no download de vídeo com qualidade {quality_option} (código {return_code})"
                )

            except Exception as e:
                self.logger.error(
                    f"Erro no download de vídeo com qualidade {quality_option}: {e}", exc_info=True
                )
                continue

        self.logger.error(
            f"Falha no download de vídeo para todas as qualidades solicitadas: {requested_quality}"
        )
