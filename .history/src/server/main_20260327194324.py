...
def _run_download_task(task_id: str, request: DownloadRequest):
    ...
    try:
        ...
        if request.mode == "video":
            ok, message, _ = downloader.download_video(
                request.url, video_quality=request.video_quality
            )
            media_type = "mp4"
        else:
        ...
        error_msg = f"Erro na Task {task_id}: {str(e)}"
        print(error_msg)
        traceback.print_exc()
...
def download_file(task_id: str):
    ...
    return FileResponse(
        ...
        media_type=f"{media_type}" if media_type else "application/octet-stream",
    )
